    ...
    try {
      ...
      try {
        await fetch(`${QRAPPSERVER_URL}/notify`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...notificationPayload, transactionId: hash })
        });
      } catch (notifyErr) {
...
export const ChargeView: React.FC<ChargeProps> = ({ onCancel, address }) => {
  ...
  const handleCharge = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Prepare Payload
      const settingsStr = localStorage.getItem('base_wallet_settings');
      // Use DEFAULT_SETTINGS if local storage is empty
      ...
      const payload = {
        creditor: {
          name: settings.businessName || "Merchant",
...
export const SettingsView: React.FC<SettingsProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    const savedSettings = localStorage.getItem('base_wallet_settings');
    if (savedSettings) {
  ...
  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    localStorage.setItem('base_wallet_settings', JSON.stringify(formData));
    setSaved(true);
